...
class NCube:
    ...
    def __post_init__(self):
        ...
        """
        # Normalize types to reduce memory: `data` -> float32, `dims` -> int8
        object.__setattr__(self, "dims", np.asarray(self.dims, dtype=np.int8))
    ...
    def marginalizar(self, ejes: NDArray[np.int8]) -> "NCube":
        ...
        if tuple(ejes) not in self.memo:
            ...
            # Ensure the marginal result is float32 to save memory
            marginal = np.mean(self.data, axis=ejes_locales, keepdims=False)
            if isinstance(marginal, np.ndarray):
                marginal = marginal.astype(np.float32)
            else:
                marginal = np.array(marginal, dtype=np.float32)
            self.memo[tuple(ejes)] = (marginal, new_dims)
        return NCube(
