...
from src.constants.base import ACTUAL, BASE_TWO, COLS_IDX, EFFECT, INT_ZERO, KEY_KPART
...
class System:
    ...
    def kpartir(
        self,
        bloques: Sequence[Sequence[tuple[int, int]]],
    ) -> "System":
        """
        Generaliza `bipartir` a `k` bloques. Es en este método donde a partir de un
        subsistema generamos una k-partición arbitraria.

        A diferencia de `bipartir` (que recibe el alcance y mecanismo de UN lado y
        asume que el complemento es el otro lado), aquí se recibe la partición
        completa como una asignación explícita de nodos a bloques. Cada nodo es una
        tupla `(tiempo, indice)` con `tiempo ∈ {ACTUAL, EFFECT}`: los presentes son
        `(ACTUAL, j)` para cada dimensión presente `j` y los futuros `(EFFECT, i)`
        para cada n-cubo `i`.

        Por cada n-cubo futuro `i` asignado a un bloque `b`, se conservan únicamente
        las dimensiones presentes `j` tales que `(ACTUAL, j)` también pertenece a `b`
        y se marginaliza el resto (las dimensiones presentes que viven en otros
        bloques), es decir `cubo.marginalizar(dims_presentes_fuera_de_b)`. Esto
        replica la semántica del corte de IIT: las conexiones que cruzan la frontera
        entre bloques se "cortan" vía marginalización.

        Con `k=2` el resultado es EXACTAMENTE el mismo que el de `bipartir`, donde el
        bloque del alcance/mecanismo equivale a uno de los dos bloques (ver test de
        consistencia).

        Args:
        ----
            bloques (Sequence[Sequence[tuple[int, int]]]): La k-partición como lista
                de bloques; cada bloque es una colección de nodos `(tiempo, indice)`.
                Todos los nodos del subsistema (presentes y futuros) deben aparecer
                exactamente en un bloque.

        Returns:
        -------
            System: El sistema correspondiente a la k-partición, sobre el que se
            obtiene la distribución marginal para compararla con el subsistema vía
            EMD y medir así la pérdida de información.
        """
        # Orden canónico de nodos y validación estructural de la partición. La
        # canonicalización vive por completo en `src/funcs/particion.py` (única
        # fuente de verdad); aquí sólo se añade el prefijo de caché `KEY_KPART`.
        orden_nodos = orden_canonico_nodos(self.dims_ncubos, self.indices_ncubos)
        validar_particion(bloques, orden_nodos)

        nuevo_sistema = System.__new__(System)
        nuevo_sistema.estado_inicial = self.estado_inicial
        nuevo_sistema.memo = self.memo

        # Mapa nodo -> identificador de bloque al que fue asignado.
        nodo_a_bloque: dict[tuple[int, int], int] = {
            (int(tiempo), int(indice)): id_bloque
            for id_bloque, bloque in enumerate(bloques)
            for (tiempo, indice) in bloque
        }
        nodos_presentes = [nodo for nodo in orden_nodos if nodo[0] == ACTUAL]

        clave = (KEY_KPART, *clave_canonica(bloques, orden_nodos))
                ...
                cubo.marginalizar(
                    np.setdiff1d(
                        ...
                        np.array(
                            [
                                j
                                for (tiempo, j) in nodos_presentes
                                if nodo_a_bloque[(tiempo, j)]
                                == nodo_a_bloque[(EFFECT, int(cubo.indice))]
                            ],
                            dtype=np.int8,
                        ),
                    )
